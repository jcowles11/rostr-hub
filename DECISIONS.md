...
## D53 — Operating Without Retained Outside Counsel (AI-Assisted Compliance Review)
**Date:** 2026-05-06 | **Status:** Active operating mode

Founder is pre-revenue and cannot afford retained outside privacy counsel ($5-15k for a written review of COMPLIANCE.md, plus ongoing engagement). Considered three options: (A) launch with NO compliance review, accept full risk; (B) launch with AI-assisted research-grade review, defensive-by-default operating principles, and explicit disclosure that no licensed counsel review has occurred; (C) delay launch until founder can afford counsel. Chose (B). Rationale: (A) is reckless even at pre-revenue scale because the marketing-vs-consent deceptive-practices risk is real and cheap to trigger inadvertently. (C) effectively kills the company — pre-revenue cash-out timeline doesn't allow indefinite delay. (B) is the path actually taken by 95%+ of pre-revenue ed-tech: ship with conservative defaults, document everything, retain counsel when revenue justifies. Specific commitments documented in COMPLIANCE.md §15: (1) marketing-↔-consent alignment as the highest residual risk, AI-reviewed before every public claim ships; (2) default-private, opt-in everything, fail-closed when ambiguous; (3) hard age floor at grade 9, geo-block CA+NY for scout features, conservative on new states; (4) every privacy-touching decision gets a DECISIONS.md entry as audit trail; (5) bounded retention with no "indefinite" claims on minor data; (6) incident response: 24h detection-to-decision, counsel + insurance on emergency retainer when triggered; (7) decline school district customers + EU/UK users until proper DPA infrastructure exists; (8) AI-assisted compliance review provides recurring work product (marketing review, pre-feature gap analysis, quarterly drift audit) but is research-grade analysis, not legal advice. Trigger to retain counsel: first paid customer OR $5k MRR OR any regulator/district inquiry. The COMPLIANCE.md disclaimer is updated to make the no-counsel-review status explicit so customers, partners, and any future regulator can see exactly what was reviewed by whom.

## D52 — Ed-Tech Posture + Geo-Block Over Bifurcation (CA + NY Scout Restriction)
**Date:** 2026-05-06 | **Status:** Decided + implemented (migration 40, middleware geo-block)

The privacy counsel review surfaced a structural conflict between Rostr's wedge (team OS for HS coaches → "designed and marketed for K-12 school purposes") and the recruiting-layer monetization (scout discovery + recruiter outreach → amassing student profiles for non-K-12 purposes). California's SOPIPA (Cal. Bus. & Prof. Code §22584) and New York's Education Law 2-d both apply to the team OS as currently positioned, and SOPIPA §22584(b)(3)–(4) prohibits exactly the kind of profile-amassing + third-party disclosure the scout layer requires. Considered three options: (A) stay an ed-tech operator, accept SOPIPA + state-law constraints, geo-block the scout/recruiter feature in CA + NY until per-state compliance work is complete; (B) bifurcate Rostr into a SOPIPA-applicable team OS (sold to schools/coaches) and a SOPIPA-exempt consumer recruiting service (sold direct-to-family with PG/PBR-style comprehensive registration agreements); (C) pivot all positioning away from K-12 ed-tech toward recruiting/development to dodge SOPIPA's scope entirely. Chose (A). Reasoning: (B) requires ~$8-15k of attorney work to draft a defensible parent-registration agreement plus 2-3 weeks of engineering to split product onboarding + data flows + marketing surfaces — not a launch-blocker line item, but expensive enough that doing it pre-revenue is malpractice. (C) blows up the wedge — coaches don't adopt a "recruiting platform" the way they adopt a team OS; PRODUCT_VISION's whole thesis depends on the coach-OS habituation. (A) is the conservative path: SOPIPA-careful team OS, geo-blocked scout layer in CA + NY, defensible at launch with the geo-block enforced at three layers (edge middleware, app service, SQL view). The trade-off is meaningful — California is the largest HS baseball market (~55k players) and we've voluntarily walled it off from monetization layer 3 (Scout) for now. Plan to revisit (B) in 6-9 months once family-pay subscription pricing is validated and the legal/engineering lift is justified by an active demand signal. The bifurcation playbook is documented in COMPLIANCE.md §6 and the strategic context in this entry; nothing else needs to happen until then.
