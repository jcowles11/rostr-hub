...
-- Drop the older overload without _recruiting_status and replace with a single comprehensive function
DROP FUNCTION IF EXISTS public.search_public_players(text, text[], integer, integer, text, text, text, integer, integer);

-- Replace the existing function with one that adds _state and _gpa_min
